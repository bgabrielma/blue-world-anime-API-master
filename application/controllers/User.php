...
class User extends BWA_Controller {
	 ...
	 */
	public function index() { parent::throwInvalidAccess(); }
	...
	public function getUser($uuid = NULL) {
		...
		if(empty($data))
			parent::throwInvalidAccess();
		else
			parent::output($status = array('code' => 200, 'message' => $data));
	}
	...
	public function getUsersByType($type = NULL) {
		...
		if(empty($query))
			...
			parent::output($status = array('code' => 200, 'message' => $query));
	}
	...
	public function insertUser($nickname, $firstname, $surname, $type_account, $email) {
		...
		if( ! $this->db->insert('senpai', $data)) {
			parent::output($status = array('code' => 500, 'message' => $this->db->error()));
		} else {
			parent::output($status = array('code' => 200, 'message' => 'ok'));
		}
	}
